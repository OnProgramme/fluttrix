...
const PhoneFrame: FC<OwnProps> =
  ({
    ...
    const currentDate = Intl.DateTimeFormat('fr', {
      hour: "2-digit",
      minute: "2-digit",
    }).format(new Date());
      ...
      return (
        <div style={containerStyle} className="flex items-center justify-center min-h-screen bg-gray-100 p-4">
          ...
          >
            ...
            {/* Bouton power */}
            <div className="h-[calc(var(--phone-width)*0.213)] w-[calc(var(--bezel-width)*0.21)] bg-gray-800 absolute -right-[calc(var(--bezel-width)*1.21)] top-[calc(var(--phone-width)*0.473)] rounded-r-md"></div>
            ...
            <div className="rounded-[2rem] overflow-hidden w-full h-full bg-white">
              ...
              <div className={
                twMerge(
                  ...
                  safeArray ? "pt-[calc(var(--phone-width)*0.054)]" : ""
                )
              }>
                {children}
              </div>
    ...
    return (
      <div style={containerStyle} className="flex items-center justify-center min-h-screen bg-gray-100 p-4">
        ...
        >
          ...
          <div className="absolute w-full top-0 bg-black/50 h-[calc(var(--phone-width)*0.06)] pr-4 rounded-t-[1.8rem] flex items-center justify-end">
          ...
          {/* Écran */}
          <div className="rounded-[1.8rem] overflow-hidden w-full h-full bg-white z-[99] ">
            {/* Zone de statut en haut pour la caméra */}
            {safeArray && <div className="w-full h-[calc(var(--phone-width)*0.06)] bg-gray-600"></div>}
            {/*Contenu */}
            <div className={
              twMerge(
                "w-full overflow-y-auto",
                safeArray ? 'h-[calc(100%-var(--phone-width)*0.06)]' : 'h-full'
              )
            }>
              {children}
            </div>
